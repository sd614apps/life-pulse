...
export default function AdminSettings({ security, onChanged }) {
  ...
  const changePassword = async (e) => {
    ...
    try {
      ...
      const updated = await base44.entities.AdminSecurity.update(security.id, {
        password_hash: hash,
        password_salt: salt,
      ...
      onChanged({ ...security, password_hash: hash, password_salt: salt, must_change_password: false, last_changed_at: updated.last_changed_at });
      setPw(''); setConfirm('');
  ...
  const toggleMfa = async (next) => {
    setBusyMfa(true);
    try {
      const updated = await base44.entities.AdminSecurity.update(security.id, { mfa_enforced: next });
      const me = await base44.auth.me();
      await base44.entities.AuditLog.create({
        event_type: 'toggle',
      ...
      if (!next) sessionStorage.removeItem('lp-admin-mfa-ok');
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-border/70 bg-card p-5">
        <h3 className="flex items-center gap-2 text-sm font-semibold text-foreground"><KeyRound className="h-4 w-4" /> Change admin password</h3>
        ...
        <form onSubmit={changePassword} className="mt-4 max-w-md space-y-3">
          <div className="space-y-1.5">
            ...
            <Input id="np" type="password" value={pw} onChange={(e) => setPw(e.target.value)} className="min-h-[48px]" required />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="nc">Confirm password</Label>
            <Input id="nc" type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} className="min-h-[48px]" required />
          </div>
          <p className="text-[11px] text-muted-foreground">Min 12 chars · uppercase · lowercase · number · special.</p>
      ...
      <div className="flex items-center justify-between gap-3 rounded-2xl border border-border/70 bg-card p-5">
        <div>
          <h3 className="flex items-center gap-2 text-sm font-semibold text-foreground"><ShieldCheck className="h-4 w-4" /> Enforced MFA</h3>
          <p className="text-xs text-muted-foreground">Require a 6-digit code on every admin entry.</p>
        </div>
        <Switch checked={!!security?.mfa_enforced} disabled={busyMfa} onCheckedChange={toggleMfa} />
      </div>
    </div>
  );
}