...
function CardRenderer({ card }) {
    ...
    return (
      <div className="rounded-xl border border-border/70 bg-card p-4">
        <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
          <Wallet className="h-4 w-4 text-emerald-500" /> Monthly spending
        </div>
        <p className="mt-1 text-2xl font-semibold text-foreground">
          {money(card.totalSpent)}{' '}
  ...
  if (card.type === 'meds') {
    return (
      <div className="rounded-xl border border-border/70 bg-card p-4">
        <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
          <Pill className="h-4 w-4 text-rose-500" /> Medications
        </div>
        {card.nextAppt && (
          <p className="mt-1 text-xs text-muted-foreground">
            Next visit: {card.nextAppt.doctor} · {formatDate(card.nextAppt.at)}
          </p>
        )}
        {card.items?.length ? (
  ...
  if (card.type === 'trip') {
    ...
    return (
      <div className="rounded-xl border border-border/70 bg-card p-4">
        <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
          <Plane className="h-4 w-4 text-sky-500" /> Travel
        </div>
        {card.trip ? (
          <div className="mt-1">
            <p className="text-lg font-semibold text-foreground">{card.trip.destination}</p>
            <p className="text-xs text-muted-foreground">
              {formatDate(card.trip.start_date)} · {days} days away
            </p>
          </div>
        ) : (
  ...
  if (card.type === 'events') {
    return (
      <div className="rounded-xl border border-border/70 bg-card p-4">
        <div className="flex items-center gap-2 text-sm font-semibold text-foreground">
          <CalendarDays className="h-4 w-4 text-violet-500" /> Family calendar
        </div>
        {card.items?.length ? (
          <ul className="mt-2 space-y-1.5">
            {card.items.map((e, i) => (
...
function ActionConfirm({ action }) {
  ...
  const confirm = async () => {
    ...
    try {
      if (action.type === 'create_appointment') {
        ...
        const list = await entities.Notification.list({ filter: { status: 'active' } });
        ...
        if (match) {
          await entities.Notification.snooze(match.id);
        } else {
          throw new Error('No matching reminder found to snooze.');
        }
      }
      setDone(true);
    } catch (e) {
      console.error('[ActionConfirm.confirm]', e);
  ...
  return (
    <div className="rounded-xl border border-border/70 bg-card p-4">
      ...
      <pre className="mt-2 max-h-32 overflow-auto rounded-lg bg-muted/60 p-2 text-xs text-foreground">
        {JSON.stringify(action.fields, null, 2)}
      </pre>
      ...
      <div className="mt-3 flex gap-2">
        ...
        <span className="inline-flex items-center text-xs text-muted-foreground">
          <X className="mr-1 h-3.5 w-3.5" /> Dismiss to cancel
        </span>
      </div>
    </div>
  );
