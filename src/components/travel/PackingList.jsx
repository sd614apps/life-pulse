...
export default function PackingList({ tripId }) {
  ...
  const load = () => {
    ...
    entities.PackingItem.list({
      filter: { trip_id: tripId },
    })
      .then((list) => setItems(list || []))
      .catch((err) => {
        console.error('[PackingList.PackingItem]', err);
        setItems([]);
      });
  };
  ...
  const toggle = async (item) => {
    setItems((prev) =>
      prev.map((p) => (p.id === item.id ? { ...p, packed: !p.packed } : p))
    );
    try {
      await entities.PackingItem.update(item.id, { packed: !item.packed });
    } catch (err) {
      console.error('[PackingList.toggle]', err);
          ...
          {items.map((i) => (
            <li key={i.id}>
              <button
                type="button"
                onClick={() => toggle(i)}
                className="flex w-full items-center gap-3 rounded-xl border border-border/70 p-3 text-left transition-colors hover:border-brand/40"
              >
                <span
                ...
                <span
                  className={`text-sm ${
                    i.packed ? 'text-muted-foreground line-through' : 'text-foreground'
                  }`}
                >
                  {i.item}
                </span>
