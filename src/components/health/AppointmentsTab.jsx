...
export default function AppointmentsTab() {
  ...
  const load = async () => {
    try {
      setAppts(await base44.entities.Appointment.list('-appointment_at', 100) || []);
    } catch {
      setAppts([]);
    }
  };
  ...
  const sendReminder = async (a) => {
    await base44.entities.Appointment.update(a.id, { reminder_sent: true });
    toast({
  ...
  const sorted = [...appts].sort((a, b) => new Date(b.appointment_at) - new Date(a.appointment_at));
  const upcoming = sorted.filter((a) => a.status === 'upcoming');
  const past = sorted.filter((a) => a.status === 'past');

  const renderList = (list, isPast) => (
    <div>
      <h3 className="mb-2 text-sm font-medium text-muted-foreground">{isPast ? 'Past visits' : 'Upcoming'}</h3>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-muted-foreground">
                  {a.member_name} · {format(parseISO(a.appointment_at), 'EEEE, d MMM yyyy · h:mm a')}
                </div>
                {a.clinic_address && (
  ...
  return (
    <div className="space-y-6">
      {upcoming.length > 0 ? renderList(upcoming, false) : (
